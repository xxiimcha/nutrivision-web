...
const Profile = () => {
  ...
  const handleUploadPicture = async () => {
    ...
    formData.append('cloud_name', 'duoqtqsuz'); // replace with your Cloudinary cloud name
  
    setProgressModalOpen(true); // Show progress modal
  
    try {
      const response = await fetch('https://api.cloudinary.com/v1_1/duoqtqsuz/image/upload', {
        method: 'POST',
        body: formData
      });
      ...
      // Send the Cloudinary URL to your backend to update the profile picture
      const updateResponse = await fetch(`${process.env.REACT_APP_API_BASE_URL}/admins/${userId}/upload-profile-picture`, {
        method: 'POST',
      ...
      setUserData({ ...userData, profilePicture: updatedData.profilePicture });
      setSelectedImage(null);
      toast.success('Profile picture updated successfully');
    } catch (error) {
      console.error(error);
      toast.error('Failed to upload and save profile picture');
    } finally {
